...
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
rm co_type.c co_type.lis co_type
echo "rm co_type.c co_type.lis co_type"
echo ""
...
echo "<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"
proc CHAR_MAP=CHARZ   iname=co_type include=$HOME/include PARSE=FULL
#proc  iname=co_type include=/usr/lib/gcc/i386-redhat-linux/4.1.1/include include=$HOME/include
...
$CCHOME/cc -o co_type co_type.c -L$HOME/lib -I$HOME/include -lCOencapi  -L$ORACLE_HOME/lib -lclntsh -lm
